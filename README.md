...
高度数组首尾各一个指针，向中间移动，同时记录左右遇到过的最高高度。如果最边高度小于右边，则移动左指针，因为这样左边如果遇到高度小于左边最高高度的那块是可以装水的，因此需要比较当前左高度与左指针遇到过的最高比较，小于则可以装当前高度的水，如果大于则更新最高高度，同理右指针高度小时也这么更新，直到两个指针相遇。
...
这题要求用固定空间和O(n)的时间复杂度，其实唯一需要考虑的0，因此先遍历一遍记录下非0数的乘积和0的索引，0只能有一个，如果多余一个直接返回全为0的结果即可。如果只有一个，那就是那个索引为非0数的乘积，其它为0。如果没有0，则得到了所有数的成绩，再遍历处于当前数即可。

### 239.Sliding Window Maximum：

感觉是我写过最简单的hard了，我是在暴力破解的基础上稍微优化了下：维护一个list，每次删掉头元素，在最后加入尾元素，期间看头元素是不是最大的，是就需要重新找list的最大值，不是的直接比较加入的和最大值就能得出新的list的大小。时间复杂度最坏的情况应该是O(nk)

这个题可以用双向队列做，不过队列中始终从左到有是递减的，然后根据队列长度是左边的出队，还是不出，我会在二刷的时候使用这个方法。
...
这是一个典型的动态规划问题，不过需要考虑的情况较多，首先对于amount：是否小于给定硬币的最小值，小于就直接返回True，如果amount在硬币中，直接返回1.然后先对硬币排序，选出小于amount的，动态规划的转移方程很简单，就不写了。
...
分治法的思想，每次用最小计数的字符做切分点，切分的字符串中找满足条件的最大长度子串，再比较。重复这个过程即可。

开始想错了，最后代码写的有点复杂，在二刷的时候我会重新修正。

### 406. Queue Reconstruction by Height:

给定一个pair的list，pair的第一个数代表身高，另一个代表这个list中它坐标应该有多少个pair的身高不小于它。要求重新排序使其满足这样的条件。首先按身高降序，编号升序排这个队列。然后开辟一个list，从左倒有遍历旧的list，按每个pair的第二个元素的值插入到新的lis中，插入完成就是最终的顺序。对于[[7,0], [4,4], [7,1], [5,0], [6,1], [5,2]],排完序后是[[7,0], [7,1], [6,1], [5,0], [5,2], [4,4]],从[7,0]开始插入，新的list为空，显然插入是第一个元素，第二个是[7,1]，显然是到1，到[6,1]后，只能有一个元素是大于它的，而新的list中有两个大于它，因此[6,1]应该在它们的中间，因此它应该插入到index为1的位置，依次类推。算法复杂度为O(nlogn)，即排序所需的时间，空间复杂度为O(n)。
...
暴力解法会超时。优化后的方案是采用字母计数：先统计出p各个字母出现的次数。然后从s中截取相同长度的字符串i统计次数，统计次数的同时要统计索引，如果当前索引的字符不在p中则下次截取i需要从当前索引+1开始，或是虽然在p中，但是i中的出现的次数已经超过i了，因此下次要从i中第一次出现这个字符的下一个开始。如果次数完全相同，这是满足条件的，加入到结果中。

在讨论区看见一个[利用ascii码做的答案](https://leetcode.com/problems/find-all-anagrams-in-a-string/discuss/92016/Python-O(n)-Time-O(1)-Space)，非常好的思路:用两个list分别代表s和p的hash表，然后统计s[:len(p)-1]的各个字母出现次数和p各个字母出现的次数。再遍历s[len(p)-1]。其中关键是要使s的hasb表中次数统计和始终与p的一致，因此有个减法的过程。

### 448. Find All Numbers Disappeared in an Array:

这题遍历这个数组，将当前数字-1的索引对应的数标记为负数。因此对于那些没有出现在数组中的数字的索引对应的是正数。

### 461.Hamming Distance:

这题是要求两个数的[汉明距离](https://en.wikipedia.org/wiki/Hamming_distance)，理解概念后就很简单，做个异操作，得到异操作的数后转换为2进制字符串得到'1'的个数即为答案。
...
树的直径问题，最大路径可能有3种情况：左子树上，右子树或者左右子树经过根节点，因此要有一个全局变量存当前最优解，然后一直用深度搜索即可。
...
这题用贪心算法是比较容易的，先用一个26个元素的数组，索引代表=char-'A',然后找到出现次数最多的课程，它出现了m次，则有(m-1)*(n+1)个间隔，然后再往这些间隔里填充即可。最终返回max((m-1)*(n+1)+count(m),len(task))，因为case中有n=0的，这个时候需要返回任务数。
